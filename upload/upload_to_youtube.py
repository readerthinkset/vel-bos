"""
YouTube Upload Script - VELOCITY BOSNIAN
"""
import os
...
def generate_video_metadata(category, num_phrases, phrases=None):
    title = f"Bosnian Learning: {num_phrases} Essential {category} Phrases"
    lines = [f"Learn Bosnian with VELOCITY BOSNIAN!", "", f"Category: {category}", "", f"Master Bosnian one phrase at a time! Today's {category} lesson:", ""]
    if phrases:
        for i, p in enumerate(phrases[:5], 0):
            lines.append(f"{i+1}. {p['english']}")
            lines.append(f"   {p.get('bosnian', '')}")
            lines.append(f"   [{p.get('transliteration', '')}]")
            lines.append("")
    lines.extend(["Tip: Repeat each phrase out loud 3 times!", "Like this video if you learned something new!", "Subscribe for daily bosnian lessons!", "", f"#LearnBosnian #BosnianLessons #BosnianForBeginners #LanguageLearning", f"#Bosnian #Education #Tutorial #DailyBosnian #{category.replace(' ', '')}", f"#VELOCITYBOSNIAN #BosnianPhrases #SpeakBosnian"])
    tags = [f"learn bosnian", f"bosnian lessons", f"bosnian for beginners", f"bosnian phrases", "language learning", f"speak bosnian", category.lower(), "education", f"daily bosnian", "velocity bosnian", f"bosnian learning"]
    return title, "\n".join(lines), tags
...
def main():
    ...
    try: upload_to_youtube(video_path=video_file, title=f"Learn Bosnian Daily", description=f"#shorts #learnbosnian #bosnian #language #education", tags=[hashtag, 'education', 'language learning'], category_id='22')
