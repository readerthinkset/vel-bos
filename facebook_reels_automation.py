"""
Facebook Reels Automation - Bilingual English/Bosnian Content Generator
IMPROVED VERSION: Better backgrounds, English categories, no repeats, VELOCITY BOSNIAN branding
...
NATIVE_VOICE = "bs-BA-GoranNeural"
...
2. Bosnian translation (in Bosnian script)
3. Transliteration (Roman script pronunciation)

Return as JSON array:
[{{"english": "...", "bosnian": "...", "transliteration": "..."}}]

IMPORTANT: Create FRESH, UNIQUE phrases that haven't been used before.
IMPORTANT: Bosnian text must be clean - no slashes, no multiple versions."""
            ...
            for p in phrases:
                ...
                if "bosnian" not in p:
                    alt_keys = ["bosnian_text", "native", "translation", p.get("language", "")]
                    for k in alt_keys:
                        if k in p:
                            p["bosnian"] = p.pop(k)
                            break
...
def generate_all_audio(phrases: list, output_dir: str):
    ...
    for i, phrase in enumerate(phrases):
        ...
        print(f"    BS: {phrase['bosnian']}")

        nat_success = asyncio.run(generate_audio_with_retries(phrase["bosnian"], NATIVE_VOICE, str(native_file)))
        if nat_success:
            print(f"    - Bosnian: {native_file.name}")
        else:
            print(f"    - Bosnian: SILENCE FALLBACK (TTS failed)")
...
def find_font(bold=False, size=40):
    ...
    font_file = FONTS_DIR / "NotoSansBosnian-Bold.ttf"
    if font_file.exists():
        try:
...
def generate_complete_image(phrase_data: dict, category_english: str, output_path: str, phrase_index: int = 0, total_phrases: int = 5):
    ...
    native = phrase_data.get("bosnian", "")
...
def generate_reel(category_english: str = None):
    ...
    for i, phrase in enumerate(phrases, 1):
        print(f"  {i}. {phrase['english']} -> {phrase['bosnian']}")
    ...
    print("\n[3/4] Generating audio (English + Bosnian with 500ms pause)...")
    ...
    print(f"  Branding: VELOCITY BOSNIAN")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print(f"VELOCITY BOSNIAN - FACEBOOK REELS AUTOMATION")
    print("="*80)
    print("\nFEATURES:")
    ...
    print(f"  - VELOCITY BOSNIAN branding at bottom")
